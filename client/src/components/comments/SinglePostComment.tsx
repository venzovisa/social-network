...
import { useCommentReactionsMutation } from "../../api/apiSlice";
...
const SinglePostComment = ({ id, author, content, createdOn, picture, likes, i, handleDeleteComment }: SinglePostCommentProps) => {
  const [openUpdate, setOpenUpdate] = useState(false);
  const { user } = useContext(AuthContext);
  const [commentReactions] = useCommentReactionsMutation();
  ...
  const handleLike = async (reaction: number) => {
    try {
      await commentReactions({ id, reaction });
    } catch (error) {
        ...
        <div className="postBottom">
          <div className="postBottomLeft">
            ...
            <img
              ...
              onClick={() => handleLike(2)} alt="Dislike" />
